...
<div id="content" class="content">
    ...
    <!-- end breadcrumb -->
    <!-- begin page-header -->
    <h1 class="page-header">Staff Attendance Rules <small>Staff Attendance Rules</small></h1>
    <!-- end page-header -->
        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-inverse">
                <div class="panel-heading">
                    ...
                    <h4 class="panel-title">Staff Attendance Rules</h4>
                </div>
                <div class="panel-body">
                    <table id="data-table" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php
                            foreach ($attendance_rules_staff as $key => $value)
                            {
                            
                            //var_dump(unserialize($value->days)[0]['weekend']);
                            
                            $days = json_decode($value->days);
                            // var_dump($days[0]->on_duty_time);

                            // $entry_time_from = date('h:i:s A', strtotime($value->days));
                            // $entry_time_to   = date('h:i:s A', strtotime($value->entry_time_to));

                            // $out_time_from = date('h:i:s A', strtotime($value->out_time_from));
                            // $out_time_to   = date('h:i:s A', strtotime($value->out_time_to));

                            echo '
                            <tr class="odd gradeX">
                                <td>'.$value->name.'</td>
                                <td>'.$days[0]->on_duty_time.'</td>
                                <td>'.$days[0]->off_duty_time.'</td>
                                <td>'.$value->department_name.'</td>
                                <td>'.($value->status ? 'Enable' : 'Disabled').'</td>
                                <td>
                                    <div class="btn-group btn-group">
                                        <a href="'.url("/admin/attendance/rule/staff/{$value->id}/view").'" class="btn btn-primary active"><i class="fa fa-cog"></i> View </a>
                                        <a href="'.url("/admin/attendance/rule/staff/{$value->id}/update").'" class="btn btn-success"><i class="fa fa-edit"></i> Update </a>
                                    </div>
                                </td>
                            </tr>
                                ';
