...
def connect(device_name=None,baud=None,timeout=None,is_com=True):
    ...
    if isFingerPrintConnected(is_com):
        try:
            ...
            print('[Connect] Not possible to connect with device %s' % (str(e)))
            pass
    return _ser
...
class FPS_GT511C3(SerialCommander):
    ...
    def __init__(self,device_name=None,baud=None,timeout=None,is_com=True):
        ...
        if not self._serial is None:
            ...
            print('[FPS_GT511C3] Not possible to connect with finger print device by %s' % self._device_name)
    ...
    def Enroll2(self):
        ...
        del cp
        self.SendCommand(packetbytes, 12)
        delay(0.4) #in Enroll2 GT511C3 take few more time to responde
        del packetbytes
        rp = self.GetResponse()
        retval = rp.IntFromParameter()
    ...
    def Enroll3(self):
        ...
        del cp
        self.SendCommand(packetbytes, 12)
        delay(0.4) #in Enroll3 GT511C3 take few more time to responde
        del packetbytes
        rp = self.GetResponse()
        retval = rp.IntFromParameter()
        retval = 3 if retval < MAX_FINGER else 0
        ...
        #     elif rp.Error == rp.errors['NACK_BAD_FINGER']:
        #         retval = 2
        #     elif retval > 0 and retval < MAX_FINGER:
        #         retval = 3
        # return retval
    ...
    def SendCommand(self, cmd,length):
        ...
        if not self._serial is None:
            ...
            if self.UseSerialDebug:
                print('[SendCommand] Can not wrie to %s' % self._device_name)
    ...
    def GetResponse(self):
        ...
        if self._serial is None:
            rp = Response_Packet()
            print('[GetResponse] Can not read from: %s' % self._device_name)
        else:
